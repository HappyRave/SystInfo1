...
"), stdout);
  ...
  -x, --decode			   decode the run length algorithm. Doesn't compose well with v,e and t\n\
"), stdout);
     ...
     bool squeeze_blank)
{
  ...
  /* Number of char to treat in the run length algorithm decoder. */
  int run;

  /* Loop in the run length algortihm decoder. */
  int i;
  ...
  while (true)
    {
      ...
      if (newlines >= 0 && number)
		...
		/* decode the run length algorithm */
...
main (int argc, char **argv)
{
  ...
  while ((c = getopt_long (argc, argv, "benstuvxAET", long_options, NULL))
         != -1)
    {
      switch (c)
        {
  ...
  do
    {
      ...
      if (! (number || show_ends || show_nonprinting
             || show_tabs || squeeze_blank || decode))
        {
      ...
      else
        {
          ...
          ok &= cat (ptr_align (inbuf, page_size), insize,
                     ...
                     decode, show_tabs, number, number_nonblank, show_ends,
                     squeeze_blank);
